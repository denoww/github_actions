...
#
# PROVENIÊNCIA — cópia fiel do gist público `denoww/5044b8da36f85afabb920263925684e9`
#   revisão de origem : 2c3a995b9988640bab0cfd1a6021f94ac82e105d (2025-09-18)
#   sha256 do original: a1f01283f75551a768c11e251fa62a83ad511c1a37df533fbad218b551fcab45
#   baixado em        : 2026-08-07
#
# Este arquivo é a FONTE DA VERDADE a partir de agora. O gist é espelho legado.
#
# ⚠️ ESTE É O SCRIPT DE MAIOR ALCANCE DO PARQUE. Roda como root, via SSM, e mexe nos
# listeners e target groups do ALB `seucondominio-web` — que é o MESMO ALB que serve o
# ERP inteiro. Os 5 repos (portaria, sc_linker, scsip, socket-server, midia_indoor_player)
# apontam para ele. Portaria ainda mexe no NLB `portaria-network`.
# Consequência: quem controlasse a fonte deste script poderia repontar o listener 443
# do ERP. Ver PSC-48 em `seguranca_sc`.
#
# Ao alterar: mude AQUI, abra PR, e só então republique o gist e bump o SHA pinado nos
# workflows `13_prod_ligar_load_balance.yml` dos 5 repos. Nunca edite o gist direto.
...
# erp_staging
##################################
# curl -fsSL https://gist.githubusercontent.com/denoww/5044b8da36f85afabb920263925684e9/raw | bash -s -- --tags "projetos=erp_staging" --instance-id  i-01bc4ddc867b24b71 --alb-name seucondominio-web --ports "https:5050:http:5050" --tcp-healthy-threshold 3 --tcp-unhealthy-threshold 3 --region us-east-1 --vpc-id vpc-0232f866 --certificado-https-arn arn:aws:acm:us-east-1:516862767124:certificate/99570c7a-1fb8-4e70-b340-637f7bcaa535
##################################
...
# Pega o primeiro valor de tag, útil como fallback
get_first_tag_value() {
  local spec="$1"
  IFS=',' read -r -a kvs <<< "$spec"
  for kv in "${kvs[@]}"; do
...
sanitize_name() {
  # troca chars inválidos por '-', colapsa '-', remove '-' nas pontas
  local s="$1"
  s="$(echo -n "$s" | tr '[:upper:]' '[:lower:]')"          # se quiser manter case, remova esta linha
  s="$(echo -n "$s" | sed -E 's/[^A-Za-z0-9-]+/-/g; s/-+/-/g; s/^-+//; s/-+$//')"
  printf '%s' "$s"
}

# Prefixos: para HTTP/HTTPS use o nome do ALB; para TCP use o nome do NLB
RAW_TAG_PREFIX="$(get_tag_value "$TAGS_SPEC" "projetos" || true)"
if [[ -n "$RAW_TAG_PREFIX" ]]; then
  TAG_PREFIX="$(sanitize_name "$RAW_TAG_PREFIX")"
fi

ALB_PREFIX_FALLBACK="$(sanitize_name "${ALB_NAME:-alb}")"
NLB_PREFIX_FALLBACK="$(sanitize_name "${NLB_NAME:-nlb}")"

TG_PREFIX_HTTP="${TAG_PREFIX:-$ALB_PREFIX_FALLBACK}"
TG_PREFIX_TCP="${TAG_PREFIX:-$NLB_PREFIX_FALLBACK}"
